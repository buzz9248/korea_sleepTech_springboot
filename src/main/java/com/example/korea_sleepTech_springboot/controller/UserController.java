...
public class UserController {
    ...
    public ResponseEntity<ResponseDto<GetUserRespDto>> getUserInfo(@AuthenticationPrincipal String userEmail) {
        // @AuthenticationPrincipal
        // : SecurityContextHolder에 저장된 인증 객체의 principal을 가져와서 사용
        // : 현재 인증된(로그인된) 사용자의 정보를 가져오는 애너테이션
        ResponseDto<GetUserRespDto> response = userService.getUserInfo(userEmail);
        return ResponseEntity.ok(response);
    }

    // 2) 회원 정보 수정
    @PutMapping(PUT_USER_INFO)
    public ResponseEntity<ResponseDto<GetUserRespDto>> updateUserInfo(@AuthenticationPrincipal String userEmail, @Valid @RequestBody UserUpdateReqDto dto) {
        ResponseDto<GetUserRespDto> response = userService.updateUserInfo(userEmail, dto);
        return ResponseEntity.ok(response);
    }

    // 3) 회원 탈퇴
    @DeleteMapping(DELETE_USER)
    public ResponseEntity<ResponseDto<Void>> deleteUser(@AuthenticationPrincipal String userEmail) {
