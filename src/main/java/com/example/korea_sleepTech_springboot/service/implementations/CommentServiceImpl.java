...
public class CommentServiceImpl implements CommentService {
    ...
    * */
    public ResponseDto<CommentRespDto> createComment(CommentCreateReqDto dto) {
        CommentRespDto respDto = null;

        // Post가 존재하는 지 확인
        D_Post post = postRepository.findById(dto.getPostId()).orElseThrow(() -> new EntityNotFoundException(ResponseMessage.NOT_EXISTS_POST + dto.getPostId()));
    ...
    @Override
    @Transactional
    public ResponseDto<CommentRespDto> updateComment(Long id, CommentUpdateReqDto dto) {
        CommentRespDto respDto = null;

        D_Comment comment = commentRepository.findById(id).orElseThrow(() -> new EntityNotFoundException(ResponseMessage.NOT_EXISTS_COMMENT + id));
