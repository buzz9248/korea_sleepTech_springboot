...
public class AdminServiceImpl implements AdminService {
    ...
    public ResponseDto<PromoteToAdminRespDto> promoteUserToAdmin(PutAuthorityReqDto dto) {
        PromoteToAdminRespDto data = null;

        // 1. 사용자 조회
        User user = userRepository.findByEmail(dto.getEmail()).orElseThrow(() -> new IllegalArgumentException(ResponseMessage.NOT_EXISTS_USER));

        // 2. 권한 생성 또는 조회
        Role adminRole = roleRepository.findByRoleName("ADMIN").orElseGet(() -> roleRepository.save(Role.builder().roleName("ADMIN").build()));

        // .anyMatch()
        //: 각 요소를 순회하면서 조건에 부합하는 요소가 단 하나라도 존재하면 true, 그렇지 않으면 false 반환
        ...
        String prevRoles = user.getRoles().stream().map(Role::getRoleName).collect(Collectors.joining(","));
        ...
        // 변경된 권한 목록 추출
        String newRoles = savedUser.getRoles().stream().map(Role::getRoleName).collect(Collectors.joining(","));

        data = new PromoteToAdminRespDto(user.getEmail(), roleList, "권한 변경이 정상적으로 이루어졌습니다.");
    ...
    @Override
    @Transactional
    public ResponseDto<DemoteToAdminRespDto> demoteUserFromAdmin(PutAuthorityReqDto dto) {
        DemoteToAdminRespDto data = null;

        User user = userRepository.findByEmail(dto.getEmail()).orElseThrow(() -> new IllegalArgumentException(ResponseMessage.NOT_EXISTS_USER));

        String prevRoles = user.getRoles().stream().map(Role::getRoleName).collect(Collectors.joining(","));

        Role adminRole = roleRepository.findByRoleName("ADMIN").orElseThrow(() -> new IllegalArgumentException("ADMIN 권한이 존재하지 않음"));

        boolean hasAdmin = user.getRoles().stream().anyMatch(role -> role.getRoleName().equals("ADMIN"));

        if (!hasAdmin) {
            throw new IllegalStateException("ADMIN 아님");
        }
        ...
        data = new DemoteToAdminRespDto(user.getEmail(), roleList, "권한 변경이 정상적으로 이루어졌습니다.");
    ...
    private String getCurrentAdminEmail() {
        ...
        // auth.getName()
        // 토큰 안에 저장된(로그인한 사용자의) username 또는 email을 반환
