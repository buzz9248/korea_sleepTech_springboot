...
public class UserServiceImpl implements UserService {
    ...
    // 1) 회원 정보 조회
    @Override
    public ResponseDto<GetUserRespDto> getUserInfo(String userEmail) {
        ...
        GetUserRespDto data = new GetUserRespDto(user);

        return ResponseDto.setSuccess(ResponseMessage.SUCCESS, data);
    }

    @Override
    public ResponseDto<GetUserRespDto> updateUserInfo(String userEmail, UserUpdateReqDto dto) {
//        User user = userRepository.findByEmail(userEmail).orElse(null);
//
//        if (user == null) {
//            return ResponseDto.setFailed(ResponseMessage.NOT_EXISTS_USER);
//        }
        ...
        User user = userRepository.findByEmail(userEmail).orElseThrow(() -> new EntityNotFoundException(ResponseMessage.NOT_EXISTS_USER));
