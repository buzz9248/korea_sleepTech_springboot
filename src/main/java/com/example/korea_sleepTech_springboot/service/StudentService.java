...
import com.example.korea_sleepTech_springboot.dto.request.StudentCreateReqDto;
import com.example.korea_sleepTech_springboot.dto.request.StudentUpdateReqDto;
...
public class StudentService {
    ...
    public List<StudentResponseDto> getAllStudents() {
        ...
        try {
            ...
            // 컬렉션 프레임워크 + stream API
            // 1) 컬렉션데이터.stream(): 스트림으로 변환
            // 2) .map(), .filter() 등 중간 연산
            // 3) list 형태로 다시 변환 - .collect(Collectors.toList());
            return studentResponseDtos;

        } catch (Exception e) {
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Error occurred while fetching student", e);
        }
    }
    ...
    public StudentResponseDto createStudent(StudentCreateReqDto studentDto) {
        StudentResponseDto studentResponseDto = null;
        try {
    ...
    public StudentResponseDto updateStudent(Long id, StudentUpdateReqDto studentDto) {
